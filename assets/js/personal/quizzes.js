 ...
 * Description: Scripts exclusive to the main Quizzes page (quizzes.html - v2.1.1).
 *              Handles quiz modal launching (category-wise and full challenge),
 *              and accordion theme toggling.
 * Dependencies: Font Awesome (for icons)
 */

document.addEventListener('DOMContentLoaded', () => {

    console.log("Rofilid Quizzes Page Script Initialized (v2.1.1 - Full Challenge Mode).");

    // --- Global Variables & Elements ---
    const body = document.body;
    const siteHeader = document.querySelector('.site-header');
    const accordion = document.querySelector('.accordion');
    const startFullChallengeBtn = document.getElementById('start-full-challenge-btn'); // New Button
    ...
    if (fullQuizData.length !== 100) {
        ...
        document.querySelectorAll('.start-quiz-btn, #start-full-challenge-btn').forEach(btn => btn.disabled = true);
    }

    // --- Helper Functions (Modal Control) ---
    function openModal(modalElement, triggerElement = null) {
        if (!modalElement) return;
        if (activeModal) closeModal(); // Close existing

        modalElement.style.display = 'flex';
        body.style.overflow = 'hidden';
        activeModal = modalElement;

        if (triggerElement && modalElement === quizModal) {
             quizTriggerElement = triggerElement;
        }
        ...
        accordionItems.forEach(item => {
            ...
            if (button && collapse) {
                button.addEventListener('click', () => {
                    ...
                    accordionItems.forEach(otherItem => {
                        if (otherItem !== item) {
                            ...
                            if (otherButton && otherCollapse && otherButton.getAttribute('aria-expanded') === 'true') {
                ...
                collapse.addEventListener('transitionend', () => {
                    if (collapse.classList.contains('show')) collapse.style.height = 'auto';
                });
            }
        });
    } else { console.error("Accordion container not found."); }
    // --- End Accordion Logic ---
    ...
    let currentQuizData = {
        ...
        score: 0,
        userAnswers: {},
        isFullChallenge: false // Flag for full challenge mode
    };
    ...
    const quizModalRestartBtn = document.getElementById('quiz-modal-restart'); // For category restart
    const quizModalRestartFullBtn = document.getElementById('quiz-modal-restart-full'); // For full challenge restart
    ...
    function startCategoryQuiz(categoryId) {
        ...
        currentQuizData = {
            ...
            score: 0,
            userAnswers: {},
            isFullChallenge: false // Mark as category quiz
        };
    ...
    function startFullChallenge() {
        console.log("Starting Full 100-Question Challenge");
        if (fullQuizData.length !== 100) {
            alert("Error: Full quiz data is incomplete. Cannot start the full challenge.");
            return;
        }
        ...
        setupQuizModalUI("Full Financial Fitness Challenge", 100); // Update title and total
        displayModalQuestion();
        openModal(quizModal, quizTriggerElement);
    }

     /**
      * Sets up the initial state of the quiz modal UI before the first question.
      * @param {string} [title='Financial Fitness Quiz'] - Optional title override.
      * @param {number} [totalQuestions=5] - Optional total questions override.
      */
    function setupQuizModalUI(title = 'Financial Fitness Quiz', totalQuestions = currentQuizData.questions.length) {
         // Reset Modal UI elements
        if (quizModalTitle) quizModalTitle.textContent = title;
        ...
        if (quizModalProgressTotal) quizModalProgressTotal.textContent = totalQuestions;

        // Hide ALL navigation buttons initially
        [quizModalNextBtn, quizModalRestartBtn, quizModalRestartFullBtn, quizModalCloseResultsBtn].forEach(btn => {
     ...
     function displayModalQuestion() {
        ...
        if (!quizModalQuestionEl || !quizModalOptionsEl || !quizModalProgressCurrent || !quizModalProgressTotal) return;

        // Update total questions display just in case it changed (e.g., switching modes)
        ...
        q.options.forEach((option, index) => {
            ...
            button.className = 'option-button btn';
     ...
     function showModalFeedback(selectedIndex, correctIndex, explanation) {
        ...
        if (quizModalNextBtn) {
            if (quiz.currentQuestionIndex < quiz.questions.length - 1) {
                ...
                setTimeout(showModalResults, 1200);
            }
        }
    }
    ...
    function showModalResults() {
        ...
        if (quizModalResultsEl) {
            ...
            const total = quiz.questions.length; // Could be 5 or 100
            const percentage = total > 0 ? Math.round((score / total) * 100) : 0;

            let feedbackMessage;
            if (quiz.isFullChallenge) {
                 feedbackMessage = 'Full Challenge Complete!';
                 if (percentage >= 90) feedbackMessage = 'Outstanding performance on the Full Challenge!';
                 else if (percentage >= 75) feedbackMessage = 'Excellent work on the Full Challenge!';
                 else if (percentage >= 50) feedbackMessage = 'Good effort on the Full Challenge! Keep learning!';
                 else feedbackMessage = 'You completed the Full Challenge! Review the insights to improve.';
                 if (quizModalTitle) quizModalTitle.textContent = "Full Challenge Results"; // Update title for results
            } else {
                 feedbackMessage = 'Keep learning and exploring!';
                 if (percentage === 100) feedbackMessage = 'Excellent work! Perfect score!';
                 else if (percentage >= 80) feedbackMessage = 'Great job! You have a strong understanding.';
                 else if (percentage >= 60) feedbackMessage = 'Good effort! Solid grasp of the basics.';
                 if (quizModalTitle) quizModalTitle.textContent = `${quiz.questions[0]?.category || 'Quiz'} Results`; // Update title
            }


            quizModalResultsEl.innerHTML = `
                <h4>${quiz.isFullChallenge ? 'Full Challenge Complete!' : 'Quiz Complete!'}</h4>
        ...
        // Show appropriate Restart button and Close button
        if (quiz.isFullChallenge) {
            if (quizModalRestartBtn) quizModalRestartBtn.style.display = 'none'; // Hide category restart
            if (quizModalRestartFullBtn) quizModalRestartFullBtn.style.display = 'inline-block'; // Show full restart
            if (quizModalRestartFullBtn) quizModalRestartFullBtn.focus(); // Focus full restart
        } else {
            if (quizModalRestartFullBtn) quizModalRestartFullBtn.style.display = 'none'; // Hide full restart
            if (quizModalRestartBtn) quizModalRestartBtn.style.display = 'inline-block'; // Show category restart
            if (quizModalRestartBtn) quizModalRestartBtn.focus(); // Focus category restart
        }
        if (quizModalCloseResultsBtn) quizModalCloseResultsBtn.style.display = 'inline-block'; // Always show close

    }

    /** Handles restarting the quiz based on whether it was a category or full challenge */
    function handleRestartClick() {
         // Hide results and buttons before restarting
         if (quizModalResultsEl) quizModalResultsEl.style.display = 'none';
         if (quizModalRestartBtn) quizModalRestartBtn.style.display = 'none';
         if (quizModalRestartFullBtn) quizModalRestartFullBtn.style.display = 'none';
         if (quizModalCloseResultsBtn) quizModalCloseResultsBtn.style.display = 'none';
        ...
        if (currentQuizData.isFullChallenge) {
            ...
            if (categoryId) {
                ...
                console.error("Cannot restart quiz: category ID not found.");
    ...
    // Listener for the NEW Full Challenge button
    if (startFullChallengeBtn) {
        startFullChallengeBtn.addEventListener('click', (e) => {
            quizTriggerElement = e.target; // Store trigger
            startFullChallenge();
        });
    } else {
    ...
    // Listeners for Category quiz start buttons inside the accordion
    if (accordion) {
        accordion.querySelectorAll('.start-quiz-btn').forEach(button => {
            button.addEventListener('click', (e) => {
                ...
                if (categoryId) {
                    ...
                    if (categoryExists) {
                        ...
                        startCategoryQuiz(categoryId); // Use specific category start function
                    } else {
                         console.error(`Data integrity issue: No questions found for categoryId ${categoryId}.`);
                         alert(`Sorry, the quiz for category ${categoryId} could not be loaded.`);
                    }
                } else {
    ...
    if (quizModalRestartBtn) quizModalRestartBtn.addEventListener('click', handleRestartClick); // Use unified handler
    if (quizModalRestartFullBtn) quizModalRestartFullBtn.addEventListener('click', handleRestartClick); // Use unified handler
    ...
    console.log("Rofilid Quizzes Page Scripts Fully Loaded and Ready.");
